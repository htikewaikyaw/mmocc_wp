		...
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $original_object && ! is_wp_error( $original_object ) ) {
					...
					// echo pll_get_term($term_id, $slug);exit;
					// $term = get_queried_object();
					// $current_slug =  $term->slug;
					// echo $current_slug;
					// $url_array = explode("/",$menu_item->url);
					// $current_lang = pll_current_language();
					// if($current_lang && $current_lang != $url_array[4]){
					// 	$url_array[4] =  $current_lang;
					// 	$url_array[7] = $current_slug;
					// }

					// $menu_item->url = implode("/",$url_array);
